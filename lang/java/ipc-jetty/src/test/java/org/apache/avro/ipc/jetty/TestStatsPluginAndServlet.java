...
public class TestStatsPluginAndServlet {
	public static GenericResponder mockGenericResponder2(Protocol local) throws Exception {
		GenericResponder mockInstance = mock(GenericResponder.class,
				withSettings().useConstructor(local).defaultAnswer(Mockito.CALLS_REAL_METHODS));
		doAnswer((stubInvo) -> {
			Object request = stubInvo.getArgument(1);
			assertEquals(0, ((GenericRecord) request).get("x"));
			return 1;
		}).when(mockInstance).respond(any(Message.class), any(Object.class));
		return mockInstance;
	}

	public static GenericResponder mockGenericResponder1(Protocol local) throws Exception {
		GenericResponder mockInstance = mock(GenericResponder.class,
				withSettings().useConstructor(local).defaultAnswer(Mockito.CALLS_REAL_METHODS));
		doAnswer((stubInvo) -> {
			Object request = stubInvo.getArgument(1);
	...
	@Test
	public void testFullServerPath() throws Exception, Exception {
		GenericResponder r = TestStatsPluginAndServlet.mockGenericResponder2(protocol);
	...
	@Test
	public void testPayloadSize() throws Exception, Exception {
		GenericResponder r = TestStatsPluginAndServlet.mockGenericResponder2(protocol);
		StatsPlugin statsPlugin = new StatsPlugin();
		r.addRPCPlugin(statsPlugin);
		Transceiver t = new LocalTransceiver(r);
	...
	public static void main(String[] args) throws Exception, Exception {
		...
		GenericResponder r = TestStatsPluginAndServlet.mockGenericResponder1(protocol);
