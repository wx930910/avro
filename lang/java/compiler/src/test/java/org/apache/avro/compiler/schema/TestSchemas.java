...
public class TestSchemas {
	...
	public static SchemaVisitor mockSchemaVisitor1() {
		SchemaVisitor mockInstance = mock(SchemaVisitor.class);
		when(mockInstance.afterVisitNonTerminal(any(Schema.class))).thenAnswer((stubInvo) -> {
			Schema terminal = stubInvo.getArgument(0);
			System.out.println("NONTerminal end: " + terminal.getFullName());
			return SchemaVisitorAction.CONTINUE;
		});
		when(mockInstance.visitNonTerminal(any(Schema.class))).thenAnswer((stubInvo) -> {
			Schema terminal = stubInvo.getArgument(0);
			System.out.println("NONTerminal start: " + terminal.getFullName());
			return SchemaVisitorAction.CONTINUE;
		});
		when(mockInstance.visitTerminal(any(Schema.class))).thenAnswer((stubInvo) -> {
			Schema terminal = stubInvo.getArgument(0);
	...
	public void textCloning() {
		...
		Schemas.visit(recSchema, TestSchemas.mockSchemaVisitor1());
	...
	public void testGetJavaClassName() {
		Assert.assertEquals("N", Schemas
				.getJavaClassName(new Schema.Parser().parse("{\"type\": \"fixed\", \"name\": \"N\", \"size\": 10}")));
		Assert.assertEquals("N", Schemas.getJavaClassName(new Schema.Parser()
				.parse("{\"type\": \"fixed\", \"name\": \"N\", \"size\": 10, \"namespace\": \"\"}")));
	...
	public void testVisit11() {
		...
		Assert.assertEquals("c1.c2.\"int\".!\"long\".!",
				Schemas.visit(new Schema.Parser().parse(s11), new TestVisitor() {
					public SchemaVisitorAction visitTerminal(Schema terminal) {
